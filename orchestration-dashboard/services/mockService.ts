...
import { DashboardMetrics, LogEntry, LogLevel, TaskStatus, TaskTrace, TimeSeriesPoint, Agent, TraceStep, ChatMessage, TopologyNode, TopologyLink } from '../types';
...
export const generateMockLogs = (count: number): LogEntry[] => {
  ...
  for (let i = 0; i < count; i++) {
    ...
    logs.push({
      ...
      level: isError ? LogLevel.ERROR : isWarn ? LogLevel.WARN : LogLevel.INFO,
